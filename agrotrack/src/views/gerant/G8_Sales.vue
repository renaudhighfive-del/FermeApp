...
async function loadSales() {
  if (campaign.value) {
    try {
      const res = await saleService.getAll({ campaign: campaign.value._id || campaign.value.id, limit: 100 })
      sales.value = res.data.sales || res.data || []
    } catch (error) {
  ...
  <div class="page-header">
    <div class="page-header-row">
      ...
      <div class="page-actions">
        ...
        <button class="btn btn-primary" @click="showModal=true" :disabled="!campaign"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Enregistrer vente</button>
      </div>
    </div>
  </div>
