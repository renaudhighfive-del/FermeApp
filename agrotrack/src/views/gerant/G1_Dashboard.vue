...
<div>
  ...
  <!-- Main Grid -->
  <div class="grid-2-1 mb-gap" v-if="activeCampaign">
    <div class="flex flex-col gap-20">
      <!-- Active Campaign Card -->
      <div class="card">
        <div class="card-header">
          <div class="card-title" style="margin-bottom:0">{{ activeCampaign.name }} <span class="badge badge-encours">{{ activeCampaign.status }}</span></div>
          <RouterLink :to="`/gerant/campaigns/${activeCampaign._id || activeCampaign.id}`" class="btn btn-outline btn-sm">Voir détail</RouterLink>
        </div>
        <div class="progress-wrap mb-12">
          <div class="progress-header">
            <span>Animaux vivants</span>
            <span class="fw-600">{{ activeCampaign.currentAnimalCount }}/{{ activeCampaign.initialAnimalCount }} ({{ getAnimalPercentage(activeCampaign) }}%)</span>
          </div>
          <div class="progress-track">
            <div class="progress-fill fill-green" :style="{width: getAnimalPercentage(activeCampaign) + '%'}"></div>
          </div>
        </div>
        <div class="progress-wrap">
          <div class="progress-header">
            <span>Budget consommé</span>
            <span class="fw-600">{{ formatCurrency(activeCampaign.spent) }}/{{ formatCurrency(activeCampaign.budget) }} ({{ getBudgetPercentage(activeCampaign) }}%)</span>
          </div>
          <div class="progress-track">
            <div class="progress-fill fill-yellow" :style="{width: getBudgetPercentage(activeCampaign) + '%'}"></div>
          </div>
        </div>
        <div class="flex gap-20 mt-auto" style="margin-top:14px;font-size:12px;color:var(--soft)">
          <span class="flex items-center gap-8">
            ...
            Démarré le {{ formatDate(activeCampaign.startDate) }}
          </span>
          <span class="flex items-center gap-8">
            ...
            Durée: {{ getDaysRemaining(activeCampaign.startDate) }} jours
          </span>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div v-else class="table-wrap">
          <table class="table">
            ...
            <tbody>
              <tr v-for="event in recentEvents.slice(0, 5)" :key="event.id">
                <td class="text-soft text-sm">{{ formatEventDate(event.date) }}</td>
                <td><span class="badge" :class="getEventBadgeClass(event.type)">{{ event.type }}</span></td>
                <td>{{ event.description }}</td>
                <td class="text-soft text-sm">{{ event.agent }}</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <div class="card" style="align-self:start">
      ...
      <div v-for="vacc in upcomingVaccinations.slice(0, 3)" :key="vacc.id" class="vacc-item">
        <div class="vacc-date-badge">
          <div class="vacc-day">{{ extractDay(vacc.date) }}</div>
          <div class="vacc-mon">{{ extractMonth(vacc.date) }}</div>
        </div>
        <div>
          <div class="vacc-name">{{ vacc.name }}</div>
          <div class="vacc-lot">{{ vacc.details }}</div>
        </div>
      </div>
    </div>
  </div>
...
async function loadData() {
  try {
    ...
    // Load events and vaccinations if needed
  } catch (error) {
